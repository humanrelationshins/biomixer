...
public class WorkerCompiler {
    /**
     * Buffers output of compilation process to specified logger via a separate
     * thread.
     */
    static class PipeOutput implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                outLogger.log(TreeLogger.ERROR,
                        "Error in reading output from compilation.", e);
            }
        }
    }
     ...
     * this is a child process, requests are sent up to parent process for it to
     * handle compilation.
     * 
     * @param logger
     * @param requests
     ...
     */
    public static SortedMap<WorkerRequestArtifact, String> run(
            TreeLogger logger, final SortedSet<WorkerRequestArtifact> requests)
            throws UnableToCompleteException {
    ...
    private static SortedMap<WorkerRequestArtifact, String> runCompiler(
        ...
        TreeLogger compLogger = logger.branch(TreeLogger.INFO,
        ...
        try {
            ...
            compLogger.log(TreeLogger.ERROR,
                    "Thread interrupted while waiting for compilation.", e);
            throw new UnableToCompleteException();
        }
        if (exitValue != 0) {
            compLogger.log(TreeLogger.ERROR,
                    "Error in compilation. See previous error.");
            throw new UnableToCompleteException();
        }
        ...
        for (WorkerRequestArtifact req : requests) {
            assert (!workerScripts.containsKey(req)) : "Module "
                    + req.getName() + " was likely compiled twice.";
            ...
            File scriptFile = new File(TEMP_WAR_DIR, name + File.separator
                    + name + WorkerRequestArtifact.WORKER_EXTENSION);
